...
### 02. Tableau 설치 및 라이선스 활성화

실습 환경을 실제로 준비하는 장입니다.  
Tableau Desktop 설치부터 Tableau Cloud 계정 준비, 추가 인증, 라이선스 활성화와 확인까지 이어지는 흐름을 다룹니다.
...
데이터 패널, 분석 패널, 선반, 시트 영역이 각각 어떤 역할을 하는지, 그리고 차원과 측정값을 어떻게 구분해야 하는지를 다룹니다.
...
막대, 라인, 맵, 파이, 트리맵 같은 기본 차트부터 정렬, 필터, 크로스탭, 스캐터 차트, 이중축까지 이어집니다.

주요 내용:

- 실습에 사용하는 Superstore 데이터 이해
...
### 08. Tableau Server / Tableau Cloud

완성된 대시보드를 조직 안에서 운영하고 협업하는 흐름을 다루는 장입니다.  
Tableau Server와 Tableau Cloud의 역할을 이해하고, 게시, 공유, 댓글, 구독, 알림, 권한, 버전 관리까지 연결합니다.
...
6장 전처리 실습까지 진행하려면 Tableau Prep도 함께 설치하는 것이 좋습니다.
...
1장부터 8장까지 순서대로 읽는 것을 추천합니다.  
개념 → 설치/활성화 → UI 이해 → 시각화 → 계산 → 데이터 준비 → 대시보드 → Server/Cloud 협업 흐름으로 이어집니다.

### 필요한 주제만 찾는 경우

아래 문서를 바로 보시면 됩니다.

- 설치/활성화: [2장](./pages/02-tableau-install-and-license.md)
- UI/기본 기능: [3장](./pages/03-tableau-ui-and-basics.md)
- 시각화: [4장](./pages/04-data-visualization.md)
- 계산식: [5장](./pages/05-calculation-and-functions.md)
- 데이터 준비: [6장](./pages/06-data-preparation.md)
- 대시보드: [7장](./pages/07-dashboard-build-and-share.md)
- Server/Cloud 협업: [8장](./pages/08-tableau-server-and-cloud.md)

### 실습 위주로 보는 경우

- `pages/` 안에서 따라하기 형태의 절부터 먼저 보셔도 됩니다.
- 데이터는 먼저 내려받아 두는 편이 좋습니다.
- 6장은 Tableau Prep, 7장은 Tableau Desktop, 8장은 Tableau Cloud 환경이 있으면 가장 자연스럽게 따라갈 수 있습니다.
...
### 2장. Tableau 설치 및 라이선스 활성화

- [2-1. Tableau Desktop 설치](./pages/02-1-tableau-desktop-installation.md)
- [2-2. Tableau 라이선스 활성화](./pages/02-2-tableau-license-activation.md)
- [2-3. Tableau Desktop Public Edition 설치](./pages/02-3-tableau-public-edition-installation.md)
...
- [4-1. 실습 데이터 소개](./pages/04-1-practice-dataset-introduction.md)
- [4-2. 다양한 차트 시각화](./pages/04-2-chart-visualization.md)
- [4-3. 정렬](./pages/04-3-sort.md)
- [4-4. 필터](./pages/04-4-filter.md)
- [4-5. 크로스탭](./pages/04-5-crosstab.md)
- [4-6. 스캐터 차트](./pages/04-6-scatter-chart.md)
- [4-7. 이중축을 활용한 시각화](./pages/04-7-dual-axis-visualization.md)
...
### 8장. Tableau Server / Tableau Cloud
