...
![LOD 개념 이미지 삽입 예정](../assets/03-4-lod-concept.png)
...
![GPT로 Tableau 계산식 생성 이미지 삽입 예정](../assets/03-4-gpt-tableau-calculation.png)
...
이 절에서는 Tableau에서 자주 사용하는 함수와 LOD 표현식을 정리했습니다.

핵심은 다음과 같습니다.

- 문자열 함수는 텍스트를 분석 가능한 구조로 바꾸는 데 유용합니다.
- 숫자 함수는 수치값을 정규화하고 비교 가능한 형태로 만드는 데 쓰입니다.
- 날짜 함수는 시계열 분석과 기간 계산의 기반이 됩니다.
- 순위 함수는 상대적 위치와 Top N 분석에 필수적입니다.
- LOD 표현식은 현재 뷰와 무관하게 원하는 집계 수준에서 계산할 수 있게 해줍니다.

즉, 함수는 계산식을 완성하는 부품이고, LOD는 그 계산이 어느 수준에서 이루어질지를 통제하는 장치입니다.
