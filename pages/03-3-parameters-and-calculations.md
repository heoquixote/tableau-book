...
이 절에서는 매개변수와 계산식을 결합해 사용자가 직접 분석 기준을 바꾸는 방법을 살펴보았습니다.

핵심은 다음과 같습니다.

- 매개변수는 사용자가 직접 지정하는 단일 값입니다.
- 계산식과 연결해야 실제로 뷰가 동적으로 바뀝니다.
- Top N, 기준선, 차원 선택, 측정값 선택 같은 상호작용에 매우 유용합니다.
- 전년비 같은 KPI 지표는 매개변수와 결합하면 사용자가 기준 연도를 직접 바꿔가며 볼 수 있습니다.

즉, 매개변수는 단순 옵션이 아니라 대시보드를 정적인 보고서에서 조작 가능한 분석 화면으로 바꾸는 핵심 도구입니다.
