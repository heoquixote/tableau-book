...
![KRSuperstore Sample 실습 데이터 소개](../assets/02-4/02-4-krsuperstore-sample.png)
...
| 주문 일자 | 고객이 주문을 생성한 날짜 | 2024-06-24 |
| 배송 일자 | 주문 상품이 실제로 배송된 날짜 | 2024-06-30 |
| 배송 방법 | 주문이 고객에게 전달되는 방식 | 표준 배송 |
| 고객 번호 | 고객을 구분하기 위한 고유 식별 번호 | SO-20335 |
...
> 날짜 컬럼은 파일에 따라 `6/24/24` 같은 미국식 표기로 저장되어 있을 수 있습니다.  
> 이 경우 Tableau 데이터 원본 화면에서 해당 필드의 데이터 유형을 `날짜`로 지정해 두면 이후 날짜 함수와 시계열 차트를 정상적으로 사용할 수 있습니다.
