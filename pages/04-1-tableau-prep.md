...
### 1-2. [실습] Tableau Prep 다운로드 및 라이선스 활성화
...
이 절에서는 Tableau Prep을 활용한 전처리 흐름을 살펴보았습니다.

핵심은 다음과 같습니다.

- Tableau Prep은 전처리 과정을 시각적인 흐름으로 설계할 수 있는 도구입니다.
- 연도별 파일 결합에는 유니온, 기준정보 추가에는 조인을 사용합니다.
- 조인 누락은 키 값 표준화 문제인 경우가 많습니다.
- 전처리 후에는 불필요한 필드를 제거해 최종 데이터셋을 정리해야 합니다.
- 마지막 출력 단계에서 `.hyper` 같은 Tableau 친화적 포맷으로 저장할 수 있습니다.

즉, Tableau Prep은 데이터를 "붙이는 도구"가 아니라, 분석 가능한 데이터 모델로 정리해 Desktop으로 넘기는 전처리 파이프라인 도구라고 이해하시면 됩니다.
