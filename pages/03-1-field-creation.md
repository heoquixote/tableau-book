...
![데이터 그룹 개념 이미지 삽입 예정](../assets/03-1-group-concept.png)
...
![권역별 매출 시각화 예시 이미지 삽입 예정](../assets/03-1-regional-group-example.png)
...
![권역 그룹 생성 이미지 삽입 예정](../assets/03-1-create-regional-group.png)

이후 기존 `시도` 필드 대신 새로 만든 `권역` 그룹으로 시각화를 바꾸면, 같은 데이터를 더 상위 수준에서 비교할 수 있습니다.

![권역 그룹 적용 이미지 삽입 예정](../assets/03-1-apply-regional-group.png)

#### 알 수 없는 항목이 보일 때

권역 그룹을 세부 정보에 넣었을 때 화면 하단에 `알 수 없는 항목`이 표시될 수 있습니다.

![알 수 없는 항목 이미지 삽입 예정](../assets/03-1-unknown-location.png)

이 경우 `위치 편집`에서 국가/지역을 `대한민국`으로 맞추면 해결되는 경우가 많습니다.

![위치 편집 이미지 삽입 예정](../assets/03-1-edit-location.png)
...
![집합 개념 이미지 삽입 예정](../assets/03-1-set-concept.png)
...
![정적 집합 이미지 삽입 예정](../assets/03-1-static-set.png)
...
![동적 집합 이미지 삽입 예정](../assets/03-1-dynamic-set.png)
...
![결합된 집합 이미지 삽입 예정](../assets/03-1-combined-set.png)
...
![집합 드릴다운 예시 이미지 삽입 예정](../assets/03-1-set-drilldown-example.png)
...
![집합 드릴다운 계산식 이미지 삽입 예정](../assets/03-1-set-drilldown-calculation.png)

이 구조의 핵심은 "집합에 포함된 항목만 하위 수준으로 펼친다"는 점입니다.  
즉, 모든 항목을 무조건 세분화하는 것이 아니라, 사용자가 관심 있는 범주만 더 깊게 보게 만드는 방식입니다.

![집합 동작 이미지 삽입 예정](../assets/03-1-set-action.png)
...
![구간 차원 크기 제안 이미지 삽입 예정](../assets/03-1-bin-size-suggestion.png)
...
![히스토그램 예시 이미지 삽입 예정](../assets/03-1-histogram-example.png)
...
### 3-3. [실습] 구간 차원을 활용하여 히스토그램 만들기

데이터 패널에서 연속형 필드를 우클릭한 뒤 `구간 만들기(Create Bins)`를 선택합니다.

![구간 만들기 이미지 삽입 예정](../assets/03-1-create-bins.png)

이후 원하는 `구간 크기(Bin Size)`를 입력합니다.

![구간 크기 입력 이미지 삽입 예정](../assets/03-1-bin-size-input.png)

그러면 새 구간 필드가 데이터 패널에 생성되고, 이를 활용해 히스토그램을 만들 수 있습니다.

![구간 필드 생성 이미지 삽입 예정](../assets/03-1-bin-field-created.png)
...
![히스토그램 완성 이미지 삽입 예정](../assets/03-1-histogram-complete.png)

## 4. 측정값과 측정값 이름

### 4-1. 측정값 이름(Measure Names)과 측정값(Measure Values)

![측정값 이름과 측정값 이미지 삽입 예정](../assets/03-1-measure-names-values.png)
...
### 4-2. [실습] 측정값과 측정값 이름 활용 예시

여러 KPI를 한 화면에 나란히 보여주고 싶을 때 `측정값 이름`과 `측정값` 조합이 매우 유용합니다.

![측정값 이름 활용 예시 이미지 삽입 예정](../assets/03-1-measure-names-example.png)
...
이 절에서는 Tableau에서 데이터를 새롭게 분류하고 표현하는 핵심 기능들을 살펴보았습니다.

핵심은 다음과 같습니다.

- 그룹은 멤버를 새로운 범주로 묶어 분석 관점을 재구성하는 기능입니다.
- 집합은 포함/제외 기준을 만들어 필터링과 드릴다운을 더 정교하게 제어합니다.
- 구간 차원은 연속형 데이터를 분포 관점에서 해석할 수 있게 해줍니다.
- 측정값 이름과 측정값은 여러 지표를 한 화면에서 함께 다룰 때 핵심적인 역할을 합니다.

즉, 이 절의 내용은 단순히 필드를 만드는 기술이 아니라, 데이터를 더 분석 가능한 형태로 바꾸는 방법이라고 보시면 됩니다.
