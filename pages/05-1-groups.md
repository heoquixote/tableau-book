...
![데이터 그룹 개념 이미지 삽입 예정](../assets/05-1/05-1-group-concept.png)
...
![권역별 매출 시각화 예시 이미지 삽입 예정](../assets/05-1/05-1-regional-group-example.png)
...
![권역 그룹 생성 이미지 삽입 예정](../assets/05-1/05-1-create-regional-group.png)

이후 기존 `시도` 필드 대신 새로 만든 `권역` 그룹으로 시각화를 바꾸면, 같은 데이터를 더 상위 수준에서 비교할 수 있습니다.

![권역 그룹 적용 이미지 삽입 예정](../assets/05-1/05-1-apply-regional-group.png)

#### 알 수 없는 항목이 보일 때

권역 그룹을 세부 정보에 넣었을 때 화면 하단에 `알 수 없는 항목`이 표시될 수 있습니다.

![알 수 없는 항목 이미지 삽입 예정](../assets/05-1/05-1-unknown-location.png)

이 경우 `위치 편집`에서 국가/지역을 `대한민국`으로 맞추면 해결되는 경우가 많습니다.

![위치 편집 이미지 삽입 예정](../assets/05-1/05-1-edit-location.png)
