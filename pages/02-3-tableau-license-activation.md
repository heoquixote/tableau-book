...
![태블로 인증 과정 이미지 삽입 예정](../assets/02-3/01-3-tableau-auth-flow-updated-v2.png)
...
![추가 인증 수단 선택 이미지 삽입 예정](../assets/02-3/01-3-auth-method-select-v2.png)

![Salesforce Authenticator 선택 화면 이미지 삽입 예정](../assets/02-3/01-3-salesforce-auth-select-v2.png)
...
![Salesforce Authenticator 설치 이미지 삽입 예정](../assets/02-3/01-3-salesforce-auth-install-v2.png)

3. `Add an Account` 또는 `계정 추가`를 눌러 계정을 추가합니다.

![Salesforce Authenticator 계정 추가 이미지 삽입 예정](../assets/02-3/01-3-salesforce-auth-add-account-v2.png)

4. 앱 화면에 표시된 두 단어를 PC 인증창에 입력합니다.

![Salesforce Authenticator 단어 인증 이미지 삽입 예정](../assets/02-3/01-3-salesforce-auth-words-v2.png)

5. 앱에서 갱신되는 확인 코드를 PC 화면에 입력합니다.

6. 확인 코드와 검증 도구 이름을 설정하면 등록이 완료됩니다.

![Salesforce Authenticator 등록 완료 이미지 삽입 예정](../assets/02-3/01-3-auth-finish-v2.png)

#### 2. Google Authenticator로 인증하는 경우

추가 인증 수단 등록 화면에서 `일회용 암호 생성기`를 선택합니다.

![일회용 암호 생성기 선택 이미지 삽입 예정](../assets/02-3/01-3-google-auth-select-v2.png)
...
![Google Authenticator 설치 이미지 삽입 예정](../assets/02-3/01-3-google-auth-install-v2.png)

![Google Authenticator 코드 추가 이미지 삽입 예정](../assets/02-3/01-3-google-auth-add-code-v2.png)

3. `QR 코드 스캔`을 눌러 PC에 표시된 QR 코드를 등록합니다.

![Google Authenticator QR 등록 이미지 삽입 예정](../assets/02-3/01-3-google-auth-qr-v2.png)

4. 앱에 표시되는 일회용 코드를 PC 화면에 입력합니다.

![Google Authenticator 확인 코드 이미지 삽입 예정](../assets/02-3/01-3-google-auth-code-v2.png)

5. 확인 코드와 검증 도구 이름을 설정하면 등록이 완료됩니다.

![Google Authenticator 등록 완료 이미지 삽입 예정](../assets/02-3/01-3-auth-finish-v2.png)
...
- 제품 키 방식은 서버 연결 없이도 활성화가 가능하다는 장점이 있습니다.
- 다만 키 관리가 필요하므로, 조직 정책에 따라 서버 로그인 방식보다 덜 선호될 수 있습니다.

## 3. 서버에 로그인하여 활성화

Tableau Desktop 설치가 끝났다면 이제 조직에서 제공받은 라이선스로 활성화합니다. 이 방식은 Tableau Cloud 계정이 먼저 활성화되어 있어야 하므로, 이메일 초대 수락부터 인증까지 한 흐름으로 진행하는 것이 좋습니다.

### 3-1. Tableau Cloud 계정 이메일 활성화
...
![Cloud 초대 메일 이미지 삽입 예정](../assets/02-3/01-3-cloud-invite-email-v2.png)

2. 메일 안의 `Join Now` 버튼을 클릭합니다.

![Join Now 버튼 이미지 삽입 예정](../assets/02-3/01-3-cloud-join-now-v2.png)

3. 이름과 비밀번호를 설정합니다.

![추가 설정 화면 이미지 삽입 예정](../assets/02-3/01-3-cloud-set-password-v2.png)

4. 설정이 완료되면 Tableau Cloud 로그인 화면으로 이동합니다.

![Cloud 로그인 완료 화면 이미지 삽입 예정](../assets/02-3/01-3-cloud-login-finish-v2.png)

이 단계가 끝나면 Tableau Cloud 계정 자체는 활성화된 상태가 됩니다. 다만 조직 정책에 따라 추가 인증 수단 등록이 필요할 수 있습니다.

### 3-2. Tableau Desktop에서 서버 로그인으로 활성화

1. Tableau 활성화 화면에서 `서버에 로그인하여 활성화`를 선택합니다.

![서버 로그인 활성화 이미지 삽입 예정](../assets/02-3/01-3-desktop-activate-server-v2.png)

2. Tableau Cloud를 선택합니다.

![Tableau Cloud 선택 이미지 삽입 예정](../assets/02-3/01-3-desktop-select-cloud-v2.png)

3. Tableau Cloud에 사용할 사용자 이름을 입력합니다.  
   이때 앞 단계에서 활성화한 Tableau Cloud 이메일을 입력합니다.

![Cloud 사용자 이름 입력 이미지 삽입 예정](../assets/02-3/01-3-desktop-cloud-username-v2.png)

4. URL 입력창이 나오면 `teamsparta`를 입력합니다.  
   표시되지 않으면 건너뜁니다.

![Site URL 입력 이미지 삽입 예정](../assets/02-3/01-3-desktop-site-url-v2.png)

5. 이메일과 비밀번호를 입력합니다.

![이메일과 비밀번호 입력 이미지 삽입 예정](../assets/02-3/01-3-desktop-email-password-v2.png)

6. Authenticator 인증을 진행합니다.

![Authenticator 인증 화면 이미지 삽입 예정](../assets/02-3/01-3-desktop-authenticator-v2.png)

7. 활성화 프로세스 완료 화면이 나타나면 `계속`을 눌러 마칩니다.

![활성화 완료 화면 이미지 삽입 예정](../assets/02-3/01-3-desktop-activation-finish-v2.png)
...
![제품 키 관리 메뉴 이미지 삽입 예정](../assets/02-3/01-3-desktop-manage-product-key-v2.png)

2. Creator 제품 키가 보이면 정상적으로 활성화된 것입니다.

![Creator 제품 키 확인 이미지 삽입 예정](../assets/02-3/01-3-desktop-product-key-check-v2.png)
