...
![데이터 모델 개요](../assets/07-1/07-1-data-model-overview.png)
...
![데이터 결합 방식 비교](../assets/07-1/07-1-data-combination-comparison.png)
...
![목표 매출 달성 여부 예시](../assets/07-1/07-1-blending-target-sales-example-v2.png)

블렌딩은 실제 매출과 목표 매출처럼 서로 다른 데이터 원본의 집계 결과를 한 시트에서 비교할 때 자주 사용합니다.

![블렌딩 메뉴](../assets/07-1/07-1-blending-menu.png)

블렌딩은 `데이터` 메뉴에서 혼합 관계를 설정해 사용할 수 있습니다.

예를 들어 `실제 매출 데이터`와 `연도별 목표 매출 데이터`가 별도 원본이라면, 연도 필드를 공통 키로 연결해 달성 여부를 분석할 수 있습니다.

![블렌딩 필드 매핑](../assets/07-1/07-1-blending-field-mapping.png)
