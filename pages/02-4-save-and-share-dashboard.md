...
![twb와 twbx 차이 이미지 삽입 예정](../assets/02-4-twb-vs-twbx.png)
...
![PowerPoint 내보내기 이미지 삽입 예정](../assets/02-4-export-powerpoint.png)
...
![이미지 내보내기 메뉴 이미지 삽입 예정](../assets/02-4-export-image-menu.png)

![PNG 저장 이미지 삽입 예정](../assets/02-4-export-image.png)
...
![크로스탭 Excel 내보내기 이미지 삽입 예정](../assets/02-4-export-crosstab.png)
...
### 2-1. Tableau Server & Cloud

조직에서 Tableau를 함께 사용한다면, 서버나 클라우드에 게시하는 방식이 가장 일반적입니다.

![Tableau Server/Cloud 게시 이미지 삽입 예정](../assets/02-4-server-cloud-publish.png)

상단 메뉴에서 `서버 -> 통합 문서 게시`를 선택하면 대시보드를 게시할 수 있습니다.

게시할 때는 일반적으로 다음 정보를 설정합니다.
...
- 사용자는 최신 버전 대시보드를 웹에서 바로 확인할 수 있습니다.
- 버전 관리와 접근 권한 통제가 쉬워집니다.
- 구독, 알림, 댓글, 협업 기능을 함께 활용할 수 있습니다.

실무에서는 파일 전달보다 게시 기반 공유가 훨씬 안정적입니다.  
같은 파일을 메일로 돌리기 시작하면 버전이 여러 개 생기고, 어떤 것이 최신인지 혼란이 생기기 쉽기 때문입니다.
...
![데이터 추출 만들기 이미지 삽입 예정](../assets/02-4-create-extract.png)

라이브와 추출의 차이는 뒤 챕터에서 자세히 다루겠지만, 여기서는 Public 게시 전에 추출 전환이 필요할 수 있다는 점만 기억하시면 됩니다.

#### 게시 화면

![Tableau Public 게시 화면 이미지 삽입 예정](../assets/02-4-public-publish-screen.png)
...
![노션 임베딩 이미지 삽입 예정](../assets/02-4-notion-embed.png)

즉, Tableau Public은 단순 게시 플랫폼을 넘어서 포트폴리오, 블로그, 노션 문서와 연결되는 외부 공유 허브 역할도 할 수 있습니다.

## 정리

이 절에서는 Tableau 결과물을 저장하고 공유하는 다양한 방식을 정리했습니다.

핵심은 다음과 같습니다.

- 로컬 공유에서는 `.twb`와 `.twbx`의 차이를 이해하는 것이 중요합니다.
- 이미지, PowerPoint, Excel 내보내기는 전달 목적에 따라 각각 장단점이 다릅니다.
- 조직 협업에는 Server/Cloud 게시 방식이 가장 안정적입니다.
- 공개 포트폴리오와 학습 공유에는 Tableau Public이 적합합니다.
