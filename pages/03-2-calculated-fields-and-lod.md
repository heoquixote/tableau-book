...
### 2-3. [실습] 행 수준 계산 vs 집계 수준 계산 시각화
...
이 절에서는 계산된 필드를 만드는 방법과 계산 수준에 따른 차이를 살펴보았습니다.

핵심은 다음과 같습니다.

- 계산된 필드는 원본 데이터를 수정하지 않고 분석용 필드를 만드는 기능입니다.
- 행 수준 계산은 개별 레코드 기준으로 먼저 계산됩니다.
- 집계 수준 계산은 요약된 결과를 기준으로 계산됩니다.
- 같은 지표라도 계산 수준이 달라지면 결과와 해석이 달라질 수 있습니다.

즉, 계산식은 단순히 "문법을 맞게 쓰는 것"이 아니라, 어떤 수준에서 의미 있는 값을 만들 것인지 결정하는 작업입니다.
