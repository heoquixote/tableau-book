...
![워크시트 필터 설정 이미지 삽입 예정](../assets/03-5/03-5-worksheet-filter-setup.png)

![년도 필터 선택 이미지 삽입 예정](../assets/03-5/03-5-year-filter-select.png)

`주문 일자` 필드를 필터로 이동시킨 뒤 `년`을 선택하고, 분석할 연도를 하나 선택합니다.

![필터 선반에 추가된 연도 필터 이미지 삽입 예정](../assets/03-5/03-5-go-dashboard.png)

이렇게 설정하면 필터 선반에 연도 필터가 추가됩니다. 이후 `매출 분석 대시보드`로 이동합니다.

![대시보드 필터 추가 이미지 삽입 예정](../assets/03-5/03-5-dashboard-add-filter.png)

총 매출 시트를 클릭한 뒤 `필터 -> 주문 일자의 연도`를 추가합니다.

![드롭다운 필터 표시 이미지 삽입 예정](../assets/03-5/03-5-filter-dropdown.png)

- 우측에 생성된 필터에서 메뉴를 열어 `단일 값(드롭다운)`을 선택합니다.
- 회색 상단 영역을 드래그하면 대시보드 안 원하는 위치로 이동할 수 있습니다.

![필터 배치 이미지 삽입 예정](../assets/03-5/03-5-filter-placement.png)

이 상태에서는 필터가 특정 워크시트에만 적용되므로, 필터 값을 바꿔도 해당 시트만 반영되는 것을 볼 수 있습니다.

![필터 적용 범위 변경 이미지 삽입 예정](../assets/03-5/03-5-apply-filter-to-all.png)
