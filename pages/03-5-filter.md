...
![워크시트 필터 영역 예시](../assets/03-5/03-5-worksheet-filter-area.png)
...
> 이 절은 대시보드가 이미 만들어져 있다는 전제로 진행합니다.  
> 대시보드를 만드는 방법 자체는 `09. 대시보드 구축 및 공유`에서 다룹니다.  
> 지금은 필터가 워크시트 단위와 대시보드 단위에서 어떻게 다르게 동작하는지 흐름만 확인하고, 9장 실습 이후에 다시 돌아와 직접 따라 하셔도 좋습니다.

대시보드에서 필터를 표시하는 작업은 단순 UI 배치가 아니라, 사용자가 직접 분석 경로를 조절하게 만드는 과정입니다.

![워크시트 필터 설정](../assets/03-5/03-5-worksheet-filter-setup.png)

![년도 필터 선택](../assets/03-5/03-5-year-filter-select.png)

`주문 일자` 필드를 필터로 이동시킨 뒤 `년`을 선택하고, 분석할 연도를 하나 선택합니다.

![필터 선반에 추가된 연도 필터](../assets/03-5/03-5-go-dashboard.png)

이렇게 설정하면 필터 선반에 연도 필터가 추가됩니다. 이후 `매출 분석 대시보드`로 이동합니다.

![대시보드 필터 추가](../assets/03-5/03-5-dashboard-add-filter.png)

총 매출 시트를 클릭한 뒤 `필터 -> 주문 일자의 연도`를 추가합니다.

![드롭다운 필터 표시](../assets/03-5/03-5-filter-dropdown.png)

- 우측에 생성된 필터에서 메뉴를 열어 `단일 값(드롭다운)`을 선택합니다.
- 회색 상단 영역을 드래그하면 대시보드 안 원하는 위치로 이동할 수 있습니다.

![필터 배치](../assets/03-5/03-5-filter-placement.png)

이 상태에서는 필터가 특정 워크시트에만 적용되므로, 필터 값을 바꿔도 해당 시트만 반영되는 것을 볼 수 있습니다.

![필터 적용 범위 변경](../assets/03-5/03-5-apply-filter-to-all.png)
