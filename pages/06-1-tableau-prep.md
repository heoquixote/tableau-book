...
![Tableau Prep 소개 이미지 삽입 예정](../assets/06-1/04-1-prep-introduction.png)
...
![Tableau Prep 개념 이미지 삽입 예정](../assets/06-1/04-1-prep-concept.png)
...
![Tableau Prep combine-clean-shape 이미지 삽입 예정](../assets/06-1/04-1-prep-combine-clean-shape.png)
...
![Tableau Prep 다운로드 페이지 이미지 삽입 예정](../assets/06-1/04-1-prep-download-page.png)

이 단계에서 Tableau.com 계정 로그인이 필요할 수 있습니다.

#### 2. 운영 체제에 맞는 설치 파일 다운로드

![설치 파일 다운로드 이미지 삽입 예정](../assets/06-1/04-1-prep-download-installer.png)
...
![등록 양식 이미지 삽입 예정](../assets/06-1/04-1-prep-registration-form.png)

설치 후 최초 실행 시 Tableau 등록 양식을 입력할 수 있습니다.

#### 5. 서버 로그인 기반 활성화 선택

![활성화 방식 선택 이미지 삽입 예정](../assets/06-1/04-1-prep-activate-server-login.png)

라이선스 활성화는 제품 키 방식이 아니라 서버 로그인 기반으로 진행할 수 있습니다.

#### 6. Tableau Cloud 선택 및 접속 정보 입력

![Tableau Cloud 선택 이미지 삽입 예정](../assets/06-1/04-1-prep-select-cloud.png)

![Tableau Cloud 사용자명 입력 이미지 삽입 예정](../assets/06-1/04-1-prep-enter-username.png)

URL 입력창이 나타나면 `teamsparta`를 입력하고, 나타나지 않으면 다음 단계로 넘어갑니다.

![Cloud URI 입력 이미지 삽입 예정](../assets/06-1/04-1-prep-enter-site-uri.png)

#### 7. 이메일/비밀번호 입력 및 인증

![이메일 비밀번호 입력 이미지 삽입 예정](../assets/06-1/04-1-prep-login.png)

![인증 진행 이미지 삽입 예정](../assets/06-1/04-1-prep-authenticator.png)

#### 8. 활성화 완료

![활성화 완료 이미지 삽입 예정](../assets/06-1/04-1-prep-activation-complete.png)
...
![실습 상황 설명 이미지 삽입 예정](../assets/06-1/04-1-prep-scenario.png)
...
![실습 원본 파일 구조 이미지 삽입 예정](../assets/06-1/04-1-prep-source-files.png)
...
![Excel, Python, SQL 전처리 대안 이미지](../assets/06-1/04-1-prep-manual-alternatives.png)

![Prep 흐름 예시 이미지 삽입 예정](../assets/06-1/04-1-prep-flow-example.png)
...
![유니온과 조인 차이 이미지 삽입 예정](../assets/06-1/04-1-prep-union-vs-join.png)
...
![유니온 시작 이미지 삽입 예정](../assets/06-1/04-1-prep-union-step-1.png)
...
![유니온 완료 이미지 삽입 예정](../assets/06-1/04-1-prep-union-step-2.png)

이 상태에서 `영업 실적 2023년`을 같은 방식으로 `유니온 1`에 추가합니다.

![유니온 확장 이미지 삽입 예정](../assets/06-1/04-1-prep-union-step-3.png)

이제 `영업 실적 2024년`도 같은 방식으로 추가하면 4개년 데이터가 하나의 유니온 흐름으로 정리됩니다.

![4개년 유니온 완료 이미지 삽입 예정](../assets/06-1/04-1-prep-union-step-4.png)
...
![불일치 컬럼 확인 이미지 삽입 예정](../assets/06-1/04-1-prep-mismatched-fields.png)
...
![불일치 필드 병합 1 이미지 삽입 예정](../assets/06-1/04-1-prep-merge-fields-discount.png)

`할인율`과 `할인`을 하나로 합치고,

![불일치 필드 병합 2 이미지 삽입 예정](../assets/06-1/04-1-prep-merge-fields-profit.png)
...
![관리자 조인 시작 이미지 삽입 예정](../assets/06-1/04-1-prep-join-manager-start.png)

`지역별 관리자` 파일을 `유니온 1` 위에 드래그해 조인합니다.

![관리자 조인 결과 이미지 삽입 예정](../assets/06-1/04-1-prep-join-manager-result.png)
...
![조인 키 수정 이미지 삽입 예정](../assets/06-1/04-1-prep-join-manager-fix.png)
...
![반품 조인 시작 이미지 삽입 예정](../assets/06-1/04-1-prep-join-returns-start.png)
...
![조인 유형 변경 이미지 삽입 예정](../assets/06-1/04-1-prep-join-type-left.png)
...
![정리 단계 추가 이미지 삽입 예정](../assets/06-1/04-1-prep-add-clean-step.png)

조인까지 끝나면 정리 단계를 추가해 최종 데이터셋을 다듬습니다.

![자동 생성 필드 제거 이미지 삽입 예정](../assets/06-1/04-1-prep-remove-table-names.png)

유니온 과정에서 자동 생성된 `Table Names` 같은 필드는 분석에 필요 없으면 제거합니다.

![조인 보조 필드 제거 이미지 삽입 예정](../assets/06-1/04-1-prep-remove-join-helper-fields.png)
...
![출력 단계 추가 이미지 삽입 예정](../assets/06-1/04-1-prep-add-output-step.png)

정리가 끝나면 출력(Output) 단계를 추가합니다.

![출력 형식 선택 이미지 삽입 예정](../assets/06-1/04-1-prep-output-options.png)
...
![hyper 출력 이미지 삽입 예정](../assets/06-1/04-1-prep-output-hyper.png)
