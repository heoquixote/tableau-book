...
라이브와 추출은 단순한 연결 방식 차이가 아니라, 데이터 신선도와 성능 사이의 트레이드오프를 어떻게 설계할 것인지에 대한 선택입니다. 실시간성이 중요하면 라이브가 적합하고, 속도와 안정성이 중요하면 추출이 더 실용적입니다.

또한 Tableau의 필터는 모두 같은 단계에서 작동하지 않습니다. 특히 Context Filter, FIXED LOD, Top N, Measure Filter, Table Calculation Filter는 계산 시점 차이를 이해해야 결과를 올바르게 해석할 수 있습니다. 실무에서 대시보드 결과가 예상과 다를 때 가장 먼저 점검해야 할 것도 바로 이 `작업 순서(Order of Operations)`입니다.
