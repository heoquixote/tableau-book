## 학습 목표

- Tableau 날짜 함수의 역할과 핵심 사용 사례를 이해합니다.
- 기준일 이동과 날짜 차이 계산을 직접 작성할 수 있습니다.
- 날짜 함수가 시계열 분석에 왜 중요한지 설명할 수 있습니다.

## 목차

1. 주문일자 1년 더하기
2. 주문일, 배송일 차이 계산
3. 날짜 함수가 실무에서 중요한 이유

날짜 함수는 시계열 분석에서 핵심입니다.  
전년비, 전월비, 평균 배송일, 고객 첫 구매일, 재구매 주기 같은 대부분의 실무 분석은 날짜 함수 위에서 돌아갑니다.

## 1. 주문일자 1년 더하기

`DATEADD`는 기준 날짜에서 일정 기간을 더하거나 빼는 함수입니다.  
`DATEADD('단위', 더할 값, [날짜 필드])` 형태로 쓰며, 단위에 따라 이동 폭이 달라집니다.

계산된 필드 하나에는 하나의 식만 들어가므로, 아래 두 예시는 각각 별도의 계산된 필드로 만듭니다.

```tableau
// C_주문일자 1년 더하기

DATEADD('year', 1, [주문 일자])
```

```tableau
// C_주문일자 9시간 더하기

DATEADD('hour', 9, [주문 일자])
```

실무에서는 비교 시점을 만들거나, 타임존 보정, 기준 기간 생성에 자주 사용합니다.

## 2. 주문일, 배송일 차이 계산

```tableau
// C_주문일, 배송일 차이 계산

DATEDIFF('day', [주문 일자], [배송 일자])
```

두 날짜 사이의 차이를 계산합니다.  
배송 리드타임, 가입 후 구매까지 걸린 시간, 문의 후 처리 시간 분석에 자주 쓰입니다.

## 3. 날짜 함수가 실무에서 중요한 이유

- 시간 간격을 기준으로 성과를 비교할 수 있습니다.
- 운영 효율성과 리드타임을 수치화할 수 있습니다.
- 특정 시점 기준의 파생 변수를 쉽게 만들 수 있습니다.

즉, 날짜 함수는 단순 날짜 계산이 아니라 "분석의 시간 축"을 만드는 역할을 합니다.
